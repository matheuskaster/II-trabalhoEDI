...
Geometria clona_forma(Geometria g, double dx, double dy) {
    if (g == NULL) {
        printf("Erro! A forma passada por parâmetro não existe. \n");
        exit (1);
    }
    ...
    if (((geometria*)g)->tipo == 'c') {
        ...
        double y_clone = y + dy;
        ULTIMO_ID++;
        return cria_forma_circulo ('c', novo_id, x_clone, y_clone, r, corp, corb);
    }
    else if (((geometria*)g)->tipo == 'r') {
        ...
        double y_clone = y + dy;
        ULTIMO_ID++;
        return cria_forma_retangulo('r', novo_id, x_clone, y_clone, w, h, corp, corb);
    }
    else if (((geometria*)g)->tipo == 'l') {
        ...
        double x1_clone = x1 + dx;
        double y1_clone = y1 + dy;
        double x2_clone = x2 + dx;
        double y2_clone = y2 + dy;
        ULTIMO_ID++;
        return cria_forma_linha('l', novo_id, x1_clone, y1_clone, x2_clone, y2_clone, cor);
    }
    else if (((geometria*)g)->tipo == 't') {
        ...
        double y_clone = y + dy;
        ULTIMO_ID++;
        return cria_forma_texto('t', novo_id, x_clone, y_clone, corp, corb, a, txto, ts);
    }
}
